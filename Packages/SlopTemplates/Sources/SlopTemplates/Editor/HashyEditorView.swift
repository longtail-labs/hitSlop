...
public struct HashyEditorView: NSViewRepresentable {
    ...
    public class Coordinator: NSObject, @preconcurrency STTextViewDelegate, @preconcurrency OverlayButtonDelegate, @preconcurrency QLPreviewPanelDataSource, @unchecked Sendable {
        ...
        private func applyImageRendering(text: String, textView: STTextView) {
            ...
            for match in matches {
                ...
                if imageURL.isFileURL {
                    ...
                    nonisolated(unsafe) let weakSelf = self as Coordinator?
                    URLSession.shared.dataTask(with: imageURL) { data, _, _ in
                        guard let data = data, let image = NSImage(data: data) else { return }
                        DispatchQueue.main.async {
                            guard let coordinator = weakSelf, coordinator.textView != nil else { return }
                            coordinator.imageCache[capturedCacheKey] = image
                            coordinator.imageFetchesInFlight.remove(capturedCacheKey)
        ...
        private func applyMarkdownLinks(in line: String, lineRange: NSRange, textView: STTextView) {
            ...
            for match in matches {
                ...
                let suffixLength = match.range.length - textRange.length - 1
        ...
        private func applyAttributes(_ attrs: [NSAttributedString.Key: Any], nsRange: NSRange, in textView: STTextView) {
            nonisolated(unsafe) let sendableAttrs = attrs
            textView.addAttributes(sendableAttrs, range: nsRange)
        }
    }
}
